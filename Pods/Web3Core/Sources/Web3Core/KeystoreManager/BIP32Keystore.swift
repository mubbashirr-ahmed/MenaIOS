...
//            guard rootNode.depth == HDNode.defaultPathPrefix.components(separatedBy: "/").count - 1 else {throw AbstractKeystoreError.encryptionError("Derivation depth mismatch")}
    ...
    private static let KeystoreParamsBIP32Version = 3
    ...
    public func createNewCustomChildAccount(password: String, path: String) throws {guard let decryptedRootNode = try? self.getPrefixNodeData(password) else {
     ...
     func encryptDataToStorage(_ password: String, data: Data?, dkLen: Int = 32, N: Int = 4096, R: Int = 6, P: Int = 1, aesMode: String = "aes-128-cbc") throws {
        if data == nil {
    ...
    public func regenerate(oldPassword: String, newPassword: String, dkLen: Int = 32, N: Int = 4096, R: Int = 6, P: Int = 1) throws {
     ...
     func getPrefixNodeData(_ password: String) throws -> Data? {
...
extension BIP32Keystore {
  public func loginWithPassword(password: String, completion: @escaping (Result<String, Error>) -> Void) {
      
        do {
            // Attempt to decrypt the keystore data with the provided password
            guard let decryptedKeyData = try self.getPrefixNodeData(password) else {
                throw NSError(domain: "Keystore", code: -1, userInfo: [NSLocalizedDescriptionKey: "Incorrect password or keystore corrupted"])
            }
            
            // Deserialize the decrypted keystore data into HDNode
            guard let rootNode = HDNode(decryptedKeyData) else {
                throw NSError(domain: "Keystore", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to deserialize root node"])
            }
            
            // Retrieve the first Ethereum address from the keystore
            guard let address = self.addresses?.first?.address else {
                throw NSError(domain: "Keystore", code: -1, userInfo: [NSLocalizedDescriptionKey: "Unable to retrieve wallet address"])
            }
            
            // Successful login, return Ethereum address
